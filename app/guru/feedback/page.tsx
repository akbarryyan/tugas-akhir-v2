...
export default async function GuruFeedbackPage({
  searchParams,
}: GuruFeedbackPageProps) {
  ...
  const feedbacks = teacherProfile
  ...
  return (
    <div className="space-y-6">
      <PageIntro
        ...
        title="Umpan Balik Mata Pelajaran"
        description="Tinjau umpan balik siswa terhadap proses pembelajaran pada mata pelajaran yang Anda ampu, lalu sesuaikan label hanya jika hasil otomatis belum menggambarkan maksud komentar dengan tepat."
      />
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <SummaryCard
          label="Umpan Balik Siap Tinjau"
          value={reviewableFeedbacks.length}
          tone="blue"
          helper="Komentar siswa yang sudah punya hasil analisis sentimen."
        />
        <SummaryCard
          label="Manual Override"
          value={manualCount}
          tone="emerald"
          helper="Komentar yang final label-nya sudah disesuaikan reviewer."
        />
        <SummaryCard
          label="Masih Otomatis"
          value={Math.max(0, reviewableFeedbacks.length - manualCount)}
          tone="amber"
          helper="Komentar yang final label-nya masih mengikuti sistem otomatis."
        />
      </section>

      <SectionCard
        title="Daftar Umpan Balik"
        description="Data yang tampil di sini dibatasi hanya untuk mata pelajaran yang Anda ampu."
      >
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <form action={reanalyzeVisibleSentimentsAction}>
            ...
            <input
              type="hidden"
              name="redirectTo"
              value={query ? `/guru/feedback?q=${encodeURIComponent(query)}` : "/guru/feedback"}
            />
            <button
              type="submit"
        ...
        <p className="mt-3 text-xs leading-6 text-slate-500">
          Reanalyze akan memperbarui hasil auto-label pada data yang tampil di mapel ampuan Anda.
          Final label manual tetap dipertahankan.
        </p>
        ...
        {reviewableFeedbacks.length === 0 ? (
          <div className="mt-5 rounded-[1rem] border border-dashed border-slate-200 bg-slate-50 px-5 py-6 text-sm leading-7 text-slate-500">
            Belum ada umpan balik pembelajaran yang bisa ditinjau pada mapel ampuan Anda.
          </div>
        ) : (
          <div className="mt-5 space-y-4">
            {reviewableFeedbacks.map((feedback) => (
              <ReviewFeedbackCard
                key={feedback.id}
                feedback={feedback}
                redirectTo={query ? `/guru/feedback?q=${encodeURIComponent(query)}` : "/guru/feedback"}
              />
            ))}
          </div>
        )}
    ...
    <article className="rounded-[1.2rem] border border-slate-200/80 bg-slate-50/70 p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="space-y-3">
          ...
          <div>
            <h3 className="text-lg font-semibold text-slate-950">{feedback.tryoutSession.tryout.title}</h3>
        ...
        <Link
          href={`/siswa/tryout/${feedback.tryoutSession.tryout.id}`}
      ...
      <div className="mt-5 grid gap-4 rounded-[1rem] border border-slate-200 bg-white p-4">
        <form action={reviewSentimentAction} className="grid gap-4">
          ...
          <div className="grid gap-4 md:grid-cols-[220px_minmax(0,1fr)]">
            <label className="grid gap-2">
              ...
              <select
                ...
                className="h-11 rounded-[0.9rem] border border-slate-200 bg-white px-3 text-sm text-slate-700 outline-none focus:border-blue-300 focus:ring-2 focus:ring-blue-100"
              >
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button
              type="submit"
              className="inline-flex h-11 items-center rounded-full bg-blue-600 px-5 text-sm font-semibold text-white transition hover:bg-blue-500"
            >
              Simpan Tinjauan
            </button>
            <span className="text-sm text-slate-500">
              Kosongkan pilihan untuk kembali memakai label otomatis.
            </span>
          </div>
        </form>

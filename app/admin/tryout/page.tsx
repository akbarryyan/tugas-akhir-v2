  ...
  return (
    <div className="space-y-8">
      ...
      >
        {tryoutRows.length === 0 ? (
          ...
          <div className="grid gap-4">
            {tryoutRows.map((tryout) => (
              <article
                key={tryout.id}
                className="rounded-[1.5rem] border border-slate-200/80 bg-[linear-gradient(180deg,#ffffff_0%,#f8fafc_100%)] p-5"
              >
        ...
        <div className="mt-5 rounded-[1.45rem] border border-dashed border-slate-200 bg-slate-50/80 p-4 text-sm leading-6 text-slate-600">
...
function TryoutSummaryCard({
  ...
  return (
    <div className="rounded-[1.7rem] border border-white/80 bg-white/88 p-5 shadow-[0_18px_54px_rgba(15,23,42,0.06)]">
      <div className="flex items-start justify-between gap-4">
        ...
        <span className={`rounded-2xl px-3 py-2 text-xs font-semibold ${accentClasses}`}>
          Aktif
        </span>
      </div>
...
function InfoPill({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-[1.2rem] border border-slate-200/80 bg-white px-3.5 py-3">
